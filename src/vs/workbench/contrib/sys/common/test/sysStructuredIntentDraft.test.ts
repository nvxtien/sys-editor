...
test('normalizes only the requirement and preserves an authoritative operation when supplied', async () => {
	...
	try {
		...
		assert.equal(request?.operation, 'BookingService.createBooking');
...
test('sends the correlation id in the body (no custom header, so no CORS preflight to fail) and traces each client stage', async () => {
	...
	try {
		await requestStructuredIntent('http://127.0.0.1:55847', 'm', 'REQ-001', 'intent', undefined, 'req-42');
...
test('explains a contract violation instead of failing silently (object-keyed inputs from a real provider)', async () => {
	...
	const traces = captureTraces();
	try {
		await assert.rejects(requestStructuredIntent('http://sidex', 'm', 'REQ-001', 'intent', undefined, 'req-7'), /does not match the Structured Intent contract \(Structured Intent has an invalid inputs\)\. Nothing was saved/);
