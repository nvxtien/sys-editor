...
describe('Sys Semantic Workbench Runtime Loading', () => {
	...
	test('L1: load resolves to READY with valid snapshot and intent state', () => {
		...
		const intentItems: SysIntentItem[] = [CINEMA_CUSTOMER_VALIDATION, CINEMA_STATUS_TRANSITION];
...
describe('Initialization Race Condition Fix', () => {
	...
	test('concurrent getIntentItems calls share initialization', async () => {
		...
		const [result1, result2, result3] = await Promise.all([getIntentItems(), getIntentItems(), getIntentItems()]);
	...
	test('storage failure rejects instead of pretending fixture data loaded', async () => {
		...
		await assert.rejects(initialize(), /Storage not available/);
	});
});
